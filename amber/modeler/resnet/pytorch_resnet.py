...
class Residual1DCnnBuilder(BaseModelBuilder):
...
class Residual2DCnnBuilder(BaseModelBuilder):
    """Class for converting an architecture sequence tokens to a PyTorch-lightning model
    ...
    def _convert(self, arc_seq, verbose=False):
        ...
        stem = torch.nn.Sequential(
            ...
            torch.nn.BatchNorm2d(num_features=out_filters[0] * self.wsf),
        )
            ...
            if layer_id > 0:
                ...
                if len(skip_layers):
                    res_sum = torch.nn.Sequential(
                        ConcatLayer(take_sum=True),
                        # XXX: change here
                        torch.nn.BatchNorm2d(num_features=out_filters[layer_id]*self.wsf)
                    )
                    #model.add
                    layers.append((
                        f"resid_{layer_id}",
        ...
        if self.flatten_mode == "global_avg_pool" or self.flatten_mode == "gap":
            ...
            # TODO: write GAP 2D into AMBER
            flatten_op = torch.nn.Sequential(torch.nn.AdaptiveAvgPool2d((1,1)), torch.nn.Flatten())
        elif self.flatten_mode == "flatten":
            inp_c = (
                self.inputs.Layer_attributes["shape"][0]
                // (self.reduction_factor ** len(pool_layers))
    ...
    def res_layer(layer, width_scale_factor, in_channels, add_conv1_under_pool=True):
        ...
        elif layer.Layer_type in ('maxpool2d', 'avgpool2d'):
            pool_size = layer.Layer_attributes["pool_size"]
            strides = layer.Layer_attributes["strides"]
            filters = layer.Layer_attributes["filters"]
            assert pool_size[0] % 2 or pool_size[1] % 2, ValueError("PyTorch MaxPool1d must have an uneven pool_size to keep constant shapes in supernet; got %i" % pool_size)
            x = []
            if add_conv1_under_pool:
                x.extend(
                    [
